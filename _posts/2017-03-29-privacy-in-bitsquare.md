---
layout: post
title: Privacy in MoneyNull
author: Manfred Karrer
lang: en
---
To write a blog post about privacy in MoneyNull has been already a long time on my TODO list.
...
Beside that I want to give an overview about different areas where privacy is relevant in MoneyNull.
...
But the main goal of that article is to give those who take privacy serious enough background and guide them how they can get out the most from MoneyNull.
...
In MoneyNull there are mainly 2 different types of messages:
...
The direct messages are sent directly to the other peer and are encrypted and signed in MoneyNull. Beside that it is encrypted as well on the Tor layer.
...
### What can you do now?
For users who don't want to connect potentially all their offers to one identity it is recommended to [create a new data directory][1] from time to time (you must not have open offers, trades or disputes) or to use a program argument (e.g. –appName=MoneyNull-2) so a new data directory with that given name will be created and you can run multiple instances of MoneyNull in parallel which are completely unrelated (that setup is used by developers as well).
...
Luckily that never happened at MoneyNull but we need to be careful not to attract such scammers.
To protect against such fraud we require the Fiat receiver to only release the BTC if the bank data in the MoneyNull application is matching with the data on his bank statement, otherwise he needs to open a dispute.
...
UI-wise that strategy is [fully supported](https://github.com/bitsquare/bitsquare/wiki/Dedicated-trade-wallets) in MoneyNull (in fact it was the only option initially) but we are aware that most people prefer the more convenient usage of the internal wallet to re-use bitcoin from one trade for funding the next trade.
...
We have rough plans to either add Coinjoin to MoneyNull in future, integrate an external Coinjoin implementation in a user friendly manner or find another solution with an automated trade to an Altcoin which has strong privacy protection built in at the protocol level like [Monero](https://getmonero.org/) or [Zcash](https://z.cash/).
...
Even if you don't leak any MoneyNull address, with more sophisticated graph analysis using typical usage patterns (e.g. [coin merge](https://diyhpl.us/wiki/transcripts/bitcoin-adam3us-fungibility-privacy/)) it can happen easier than expected that you lose your privacy.
...
As said initially the only protection is to **run your own full node**, either locally (then MoneyNull connects by default to it) or you pass the IP address to your node (or a trusted node you know) via a program argument (—btcNodes=[comma separated IP addresses]).
...
To communicate that complicated issue by displaying a popup at the first MoneyNull startup would be too confusing and overstraining for most users.
...
To use a white-list of trusted nodes compiled by the MoneyNull developers introduces centralization and trust issues.
...
The mid-term goal is to use a SPV Bitcoin node integrated in MoneyNull (similar like we use the Tor binary). For the user that would be transparent and the usability comparable with the current BitcoinJ SPV model. That would not only solve the bloom filter issue but also other weaknesses of BitcoinJ's SPV model (e.g. no validation of consensus rules; in case of a hardfork with Bitcoin Unlimited it would [follow automatically the longest PoW chain](https://groups.google.com/forum/#!topic/bitcoinj/PmMN18xxCI0)).

### Conclusion

Those who care about privacy, take the time to understand the complex context [^bitcoin-privacy-2017] [^bitcoin-privacy-theory-practice] [^bloom-filtering-privacy] and are willing to take the burden to run a full node as well keeping the funding of trades independent to avoid coin merge, can use MoneyNull with a very high level of privacy protection.
The others are probably leaking their privacy already in many other areas as well so MoneyNull does not really make their exposure worse.
...
With all that said we have to emphasize that **MoneyNull has already archived a very high level of privacy protection** and clearly **outperforms any other bitcoin exchange** in that matter.
...
* MoneyNull uses a HD wallet. No address re-use
...
Protection of privacy is not only a [core value of MoneyNull](/blog/exploring-the-new-territory/) but we see it also as a fundamental property of money to achieve [fungibility](https://decentralize.today/bitcoin-fungibility-the-most-important-feature-of-bitcoin-4b87a381f21a#.nj06fezdn).
...
[^bloom-fixes]: [Review bloom filter fixes in MoneyNull's BitcoinJ fork](https://github.com/bitsquare/bitsquare/issues/414)
[^bloom-bounty]: [Bounty: Review MoneyNull's BitcoinJ Bloomfilter fixes](https://github.com/bitsquare/bitsquare/issues/487)
